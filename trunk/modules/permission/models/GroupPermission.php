...
class Models_GroupPermission extends Nine_Model
{ 
    /**
    ...
    {    	
        $select = " SELECT  `gp` . * ,  `p` . * 
                    FROM (SELECT * FROM `{$this->_prefix}permission` WHERE module = " . $this->getAdapter()->quote($access) . ") AS  `p`
                    LEFT JOIN (SELECT * FROM `{$this->_prefix}group_permission` WHERE group_id = " . $this->getAdapter()->quote($groupId) . ") AS  `gp` 
