                            ...
                            <fieldset> <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
                                <p>
                                    ...
                                    {{foreach from=$allCats item=item}}
                                        <option value="{{$item.content_category_gid}}" {{if $data.parent_id == $item.content_category_gid}}selected="selected"{{/if}}>{{$item.name}}</option>
                                    {{/foreach}}
                                    </select> 
                                </p>
                                <p>
                                    <label>Sorting<span class="red">*</span><br/><br/></label>
                                        <input class="text-input small-input" type="text"  name="data[sorting]" value="{{$data.sorting}}" />{{if $errors.repeat_password}}<span class="input-notification error png_bg"></span>{{/if}}
                                        <br /><small>No 1 will display as first article</small>
                                </p>
                                {{if 1 == $data.content_deleteable}}
                                <p>
                                    <label>Enabled<span class="red">*</span></label>
                                ...
                                {{/if}}
                                <p>
                                    <label>Content Deleteable<span class="red">*</span></label>
                                    <input type="radio" name="data[content_deleteable]" value="1" {{if $data.content_deleteable != '0'}}checked="checked"{{/if}}/> Yes &nbsp;&nbsp;&nbsp;&nbsp;
                                    <input type="radio" name="data[content_deleteable]" value="0" {{if $data.content_deleteable == '0'}}checked="checked"{{/if}}/> No
                                	<br /><small>Yes: user can delete this category & its content. No: can not delete</small>
                                </p>
         ...
         <!-- END IMAGES -->                       
                    ...
                    <div class="tab-content {{if $fullPermisison || !$smarty.foreach.langDiv.first}}hidden{{/if}}" id="tab{{$item.lang_id}}">
                            ...
                            <fieldset> <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
                                ...
                                <p>
                                    <label>Name</label>
                                        <input id="name{{$smarty.foreach.langDiv.iteration}}" class="text-input large-input" type="text"  name="data[{{$item.lang_id}}][name]" value="{{$data[$item.lang_id].name}}" />
                                </p>
                                <p>
                                ...
                                <p>
                                    <div><textarea style="float:left;" class="text-input textarea ckeditor"  name="data[{{$item.lang_id}}][description]" rows="20" cols="90">{{$data[$item.lang_id].description}}</textarea></div>
