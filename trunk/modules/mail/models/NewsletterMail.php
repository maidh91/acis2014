...
class Models_NewsletterMail extends Nine_Model
{ 
    /**
     ...
     * @var mixed
     */
    protected $_primary = 'newsletter_mail_id';
    /**
    ...
    {
        $this->_name = $this->_prefix . 'newsletter_mail';
        return parent::__construct($config); 
    }
	
    //public function sendHtmlMail($data = array(), $to = array(), $subject = null)
    public function sendHtmlMail($content, $data = array(), $to = array(), $subject = null)
    {
        if (!is_array($to)) {
            $to = array($to);
        }
        ...
        $mail['content'] = str_replace(array_keys($data), $data, $content);
	...
	public function getAllNewsLetterMails($condition = array(), $order = null, $count = null, $offset = null)
    {
        $select = $this->select()
                ->order($order)
                ->limit($count, $offset);
