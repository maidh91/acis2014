                ...
                <div class="content-box-content">
                    ...
                    <div class="tab-content default-tab" id="tab1"> <!-- This is the target div. id must match the href of this div's tab -->
                        ...
                        <table>
                            ...
                            {{foreach from=$allMails item=item name=mail}}
                                <tr>
                                    <td><input type="checkbox" value="{{$item.mail_gid}}" name="allMails"/></td>
                                    <td>{{$smarty.foreach.mail.iteration}}</td>
                                    <td>{{$item.name}}</td>
                                    
                                    <td colspan="4">
                                    <!-- All languages -->
                                        <table>
                                            <tbody id="table{{$item.mail_gid}}">
                                                <tr>
                                                    <td width="60%">--</td>
                                                    <td width="10%" class="center">
                                                    	{{if $fullPermisison}}
                                                        <a href="{{$APP_BASE_URL}}mail/admin/edit-system-mail/gid/{{$item.mail_gid}}" title="Edit"><img src="{{$LAYOUT_HELPER_URL}}admin/images/icons/pencil.png"  alt="Edit" /></a>
                                                    	{{else}}
                                                    	--
                                                    	{{/if}}
                                                    </td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="10%" class="center">
                                                    	{{p name=edit_system_mail module=mail expandId=$langId}}
                                                        <a href="{{$APP_BASE_URL}}mail/admin/edit-system-mail/gid/{{$item.mail_gid}}/lid/{{$item2.lang_id}}" title="Edit"><img src="{{$LAYOUT_HELPER_URL}}admin/images/icons/pencil.png"  alt="Edit" /></a>
                                                    	{{/p}}
                                                    	{{np name=edit_system_mail module=mail expandId=$langId}}
                                                        --
                                                        {{/np}}
                                                    </td>
                                                    <td width=10%" class="center">{{$item2.mail_id}}</td>
